...
| `--unicode` "may be deferred as a whole" | **Shipped, on a narrower footing than "every Unicode block that beats ASCII."** `render::tiles::unicode_glyph` is a complete, hand-picked 24-glyph table restricted to characters individually verified `East_Asian_Width=Neutral/Narrow` (never `Ambiguous`/`Wide`), so it cannot shear the fixed tile grid. `--ascii`/`--unicode` are mutually exclusive; no `unicode-width` runtime dependency (`Cargo.toml` stays fixed — the table is compile-time, not a width probe). | `docs/user/cli.md`, "The Unicode decision"; `.autodev/DECISIONS.md`. |
